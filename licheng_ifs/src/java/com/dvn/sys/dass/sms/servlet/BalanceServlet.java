...
public class BalanceServlet extends BaseServlet {
	...
	private SMSBalanceAPIRequest requestAPI;

	private SMSBalanceAPIResponse responseAPI;
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (null == responseAPI) {
			responseAPI = new SMSBalanceAPIResponse();
		}
		responseAPI.setReturnCode(returnCode);

		byte[] responseContent = putResponseAPI(encoding);
		logger.debug(LogPrinter.printLogForResponse("Balance", Tool
	...
	private void execute(String transactionId) {
		...
		FunctionResult balanceRes = smsFuncMana.callFuncBalanceQuery("SMS-FEP", uuid,
				transactionId, remark);
		responseAPI = new SMSBalanceAPIResponse();
		responseAPI.setBalance("".equals(balanceRes.getAccountValue())?"REAL=0;SUMNOPAID=0|VIRTUAL=0;SUMNOPAID=0":balanceRes.getAccountValue());
		returnCode = ResultDefine.transResultCode(balanceRes.getResultCode());
	...
	private void putRequestAPI(String requestContent) {
		try {
			requestAPI = (SMSBalanceAPIRequest) getRequest(requestContent,
					com.dvn.sys.dass.sms.po.balance.request.ObjectFactory.class);
		} catch (IOException e) {
	...
	private byte[] putResponseAPI(String encoding) {
		byte[] temp = null;
		try {
			temp = putResponse(responseAPI, com.dvn.sys.dass.sms.po.balance.response.ObjectFactory.class, encoding);
		} catch (JAXBException e) {
