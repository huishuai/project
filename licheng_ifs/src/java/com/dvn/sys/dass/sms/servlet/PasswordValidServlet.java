...
/**
 */
public class PasswordValidServlet extends BaseServlet {

	private static final Logger logger = Logger
			.getLogger(PasswordValidServlet.class);
	...
	private SMSPwdValAPIRequest requestAPI;

	private SMSPwdValAPIResponse responseAPI;
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		logger.error("###PasswordValidServlet###");
		...
		if (null == responseAPI) {
			responseAPI = new SMSPwdValAPIResponse();
		}
		responseAPI.setReturnCode(returnCode);

		byte[] responseContent = putResponseAPI(encoding);
		logger.debug(LogPrinter.printLogForResponse("PwdVal", Tool
	...
	private void execute(String transactionId) {
		...
		String password = requestAPI.getPassword();
		
		SMSFuncManager smsFuncMana = (SMSFuncManager)getBean("SMSFuncManager");
		FunctionResult pwdValRes = smsFuncMana.callFuncPasswordVerify("SMS-FEP",
				uuid, transactionId, remark, password);
		responseAPI = new SMSPwdValAPIResponse();
		if(!"100".equals(pwdValRes.getResultCode()) && !"101".equals(pwdValRes.getResultCode())){
			returnCode = ResultDefine.transResultCode(pwdValRes.getResultCode());
		}
	...
	private void putRequestAPI(String requestContent) {
		try {
			requestAPI = (SMSPwdValAPIRequest) getRequest(requestContent,
					com.dvn.sys.dass.sms.po.pwdval.request.ObjectFactory.class);
		} catch (IOException e) {
	...
	private byte[] putResponseAPI(String encoding) {
		byte[] temp = null;
		try {
			temp = putResponse(responseAPI, com.dvn.sys.dass.sms.po.pwdval.response.ObjectFactory.class, encoding);
		} catch (JAXBException e) {
